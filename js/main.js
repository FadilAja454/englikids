...
let profilSiswa     = { nama: 'Anak', avatar: '🦉', kelas: '1-2' };
...
async function loadDataSoal() {
    try {
        ...
        const warnaPerKategori = {
            ...
            kalimat : 'linear-gradient(90deg,#6BCB77,#4DA8DA)',
        };

        let allSoal = json.soal.map(soal => ({
            ...soal,
        ...
        // Filter by class
        dataSoal = allSoal.filter(s => s.kelas === profilSiswa.kelas);
        if (dataSoal.length === 0) dataSoal = allSoal; // fallback

        console.log(`[App] ✓ ${dataSoal.length} soal dimuat untuk kelas ${profilSiswa.kelas}.`);
    } catch (err) {
...
function tampilkanSoal(idx) {
    ...
    } else if (soal.tipe === 'susun_kata') {
        tampilkanSoalSusunKata(soal);
    } else {
...
function tampilkanSoalUcapkan(soal) {
    ...
    document.getElementById('section-susun-kata')?.classList.add('hidden');
...
// ── Soal tipe SUSUN KATA ─────────────────────
let stateSusunKata = { jawaban: [], pilihan: [] };

function tampilkanSoalSusunKata(soal) {
    document.getElementById('section-ucapkan').classList.add('hidden');
    document.getElementById('section-cocokkan').classList.add('hidden');
    document.getElementById('section-susun-kata').classList.remove('hidden');

    document.getElementById('susun-kata-terjemahan').textContent = soal.terjemahan;
    document.getElementById('susun-kata-emoji').textContent = soal.emoji || '🧩';
    document.getElementById('badge-susun-kata').style.background = soal.warnaBadge;

    document.getElementById('feedback-susun-kata').classList.add('hidden');
    document.getElementById('area-mic-susun-kata').classList.add('hidden');
    document.getElementById('area-pilihan-kata').classList.remove('hidden');
    document.getElementById('susun-kata-instruksi').classList.remove('hidden');

    stateSusunKata.jawaban = [];
    // Shuffle the kata_acak array again just to be safe
    stateSusunKata.pilihan = [...soal.kata_acak].sort(() => Math.random() - 0.5);

    renderSusunKataUI();
}

function renderSusunKataUI() {
    const areaPilihan = document.getElementById('area-pilihan-kata');
    const areaJawaban = document.getElementById('area-jawaban');
    
    areaPilihan.innerHTML = '';
    areaJawaban.innerHTML = '';

    // Render Pilihan
    stateSusunKata.pilihan.forEach((kata, i) => {
        const btn = document.createElement('button');
        btn.className = 'kata-btn';
        btn.textContent = kata;
        btn.onclick = () => {
            stateSusunKata.jawaban.push(kata);
            stateSusunKata.pilihan.splice(i, 1);
            renderSusunKataUI();
            cekJawabanSusunKata();
        };
        areaPilihan.appendChild(btn);
    });

    // Render Jawaban
    stateSusunKata.jawaban.forEach((kata, i) => {
        const btn = document.createElement('button');
        btn.className = 'kata-btn';
        btn.textContent = kata;
        // Klik di jawaban mengembalikan ke pilihan
        btn.onclick = () => {
            stateSusunKata.pilihan.push(kata);
            stateSusunKata.jawaban.splice(i, 1);
            renderSusunKataUI();
        };
...
function cekJawabanSusunKata() {
    const soal = dataSoal[indexSoalAktif];
    if (stateSusunKata.pilihan.length === 0) {
        const jawabanUser = stateSusunKata.jawaban.join(' ');
        if (jawabanUser === soal.kata) {
            // Benar!
            const fb = document.getElementById('feedback-susun-kata');
            fb.textContent = `✅ Benar!`;
            fb.style.background = '#6BCB77';
            fb.classList.remove('hidden');
            
            // Tampilkan Mic
            document.getElementById('area-pilihan-kata').classList.add('hidden');
            document.getElementById('susun-kata-instruksi').classList.add('hidden');
            document.getElementById('area-mic-susun-kata').classList.remove('hidden');
            document.getElementById('area-mic-susun-kata').classList.add('flex');
            
            // Disable further edits
            const areaJawaban = document.getElementById('area-jawaban');
            Array.from(areaJawaban.children).forEach(btn => btn.onclick = null);
        } else {
            // Salah
            tampilkanToast(`❌ Urutan belum tepat. Coba lagi!`, '#FF6B6B');
        }
    }
}
...
function tampilkanVoiceTranscript(teks, isFinal) {
    // Untuk soal biasa
    let container = document.getElementById('voice-transcript');
    let textEl    = document.getElementById('transcript-text');
    
    // Jika soal susun kata sedang aktif
    if (dataSoal[indexSoalAktif]?.tipe === 'susun_kata') {
        container = document.getElementById('voice-transcript-susun');
        textEl    = document.getElementById('transcript-text-susun');
    }
...
function tampilkanMatchTranscript(cocok, kataSoal) {
    let matchDiv = document.getElementById('transcript-match');
    let iconEl   = document.getElementById('match-icon');
    let textEl   = document.getElementById('match-text');
    
    // Jika soal susun kata sedang aktif
    if (dataSoal[indexSoalAktif]?.tipe === 'susun_kata') {
        matchDiv = document.getElementById('transcript-match-susun');
        iconEl   = document.getElementById('match-icon-susun');
        textEl   = document.getElementById('match-text-susun');
    }
...
function resetVoiceTranscript() {
    const ids = ['voice-transcript', 'transcript-match', 'voice-transcript-susun', 'transcript-match-susun'];
    ids.forEach(id => {
...
function inisialisasiWaveformCanvas() {
    // Will dynamically select the correct canvas based on active section
}

// Mulai render waveform dari AnalyserNode DSP
function mulaiRenderWaveform(analyserNode) {
    waveformCanvas = document.getElementById('waveform-canvas');
    let bars = document.getElementById('waveform-bars');
    
    if (dataSoal[indexSoalAktif]?.tipe === 'susun_kata') {
        waveformCanvas = document.getElementById('waveform-canvas-susun');
        bars = document.getElementById('waveform-bars-susun');
    }
...
function berhentiRenderWaveform() {
    ...
    let canvasSusun = document.getElementById('waveform-canvas-susun');
    
    if (canvas) { canvas.classList.remove('active'); canvas.classList.add('hidden'); }
    if (canvasSusun) { canvasSusun.classList.remove('active'); canvasSusun.classList.add('hidden'); }
    ...
    try {
        ...
        await EngliKidsDSP.startRecording({
            ...
            onVolumeUpdate: (rms) => {
                ...
                const barsSelector = dataSoal[indexSoalAktif]?.tipe === 'susun_kata' 
                    ? '#waveform-bars-susun .wave-bar' 
        ...
        try {
            ...
            const barsId = dataSoal[indexSoalAktif]?.tipe === 'susun_kata' ? 'waveform-bars-susun' : 'waveform-bars';
            const barsEl = document.getElementById(barsId);
            if (barsEl) {
                barsEl.classList.remove('hidden');
                barsEl.classList.add('flex');
            }
        }

        uiSedangRekam = true;
        
        const isSusunKata = dataSoal[indexSoalAktif]?.tipe === 'susun_kata';
        const btnMicId = isSusunKata ? 'btn-mic-susun' : 'btn-mic';
        const instruksiId = isSusunKata ? 'teks-instruksi-susun' : 'teks-instruksi';
        
        document.getElementById(btnMicId)?.classList.add('recording');
        const instruksi = document.getElementById(instruksiId);
        if (instruksi) {
            instruksi.textContent = '🎙️ Sedang merekam... ucapkan!';
            instruksi.style.color = '#FF6B6B';
        }
...
async function stopRekam() {
    ...
    const btnMicId = isSusunKata ? 'btn-mic-susun' : 'btn-mic';
    const instruksiId = isSusunKata ? 'teks-instruksi-susun' : 'teks-instruksi';
    const barsId = isSusunKata ? 'waveform-bars-susun' : 'waveform-bars';

    document.getElementById(btnMicId)?.classList.remove('recording');
    berhentiRenderWaveform();
    document.getElementById(barsId)?.classList.add('hidden');
    
    const instruksi = document.getElementById(instruksiId);
    if (instruksi) {
        instruksi.textContent = '⏳ Menganalisa suaramu...';
        instruksi.style.color = '#A06CD5';
    }
...
function tampilkanModalSkor() {
    ...
    const soalUcapkan = dataSoal.filter(s => s.tipe === 'ucapkan' || s.tipe === 'susun_kata').length;
